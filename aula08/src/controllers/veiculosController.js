...
const veiculosController = {
	listarTodos: async (req, res) => {
		try {
			...
			res.status(500).json({ erro: "Erro ao consutarbanco de dados." });
		}
	},
...
db('veiculos').where('id', id).first();
			...
			if (erro.messege.includes('UNIQUE constraint failed')) {
				return res.status(409).json({ erro: "J a existe um veiculo cadastrado com essa placa" });
			}
